...
import type { TopPeriodsResult, TopPeriodsByMetricArguments } from '@/types/graphql.types';
...
function StatOutput({
  ...
  return (
    <Typography variant="body2">
      <b>{data.metricValue}{showUnit ? ` ${definition.unit}` : ''}</b> {connectingVerbMap[definition.temporalUnit] as string}{' '}
      {formatDate(
...
function hasData(headlineStat: HeadlineStat): headlineStat is HeadlineStat & { data: TopPeriodsResult } {
  return headlineStat.data !== undefined;
}
function AccordionItem({ headlineStat, onExpanded, expandedId }: { headlineStat: HeadlineStat, onExpanded: (id: string) => void, expandedId: string | false }) {
  ...
  return (
    <Accordion onChange={onChange} expanded={expandedId === headlineStat.definition.id}>
      ...
      <AccordionDetails id={`${headlineStat.definition.id}-content`}>
        {hasData(headlineStat) ? <List component="ol">
          <ListItem disablePadding>
            <ListItemText>
              <StatOutput data={headlineStat.data} definition={headlineStat.definition} showUnit={true} />
            </ListItemText>
          </ListItem>
        </List> :
...
export  function StatsAccordion({data}: {data: HeadlineStat[] | undefined}) {
  ...
  return <>
    {data ? data.map((item) => (
      <AccordionItem key={item.definition.id} headlineStat={item} onExpanded={(id) => setExpanded(id)} expandedId={expanded} />
    )) : <Typography component="span">No data available</Typography>}
  </>;
}
