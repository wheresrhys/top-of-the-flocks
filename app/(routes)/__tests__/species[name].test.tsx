...
describe('species page', () => {
  it('should show correct heading', async () => {
    render(await Page({
      params: new Promise((resolve) => resolve({ speciesName: 'Cetti\'s Warbler' })) }));
  ...
  it('should show headline stats', async () => {
    render(await Page({ params: new Promise((resolve) => resolve({ speciesName: 'Cetti\'s Warbler' })) }));
    ...
    expect(statsLineItems[0].textContent).toBe('6 individuals, 15 encounters, caught at 6 sessions');
    expect(statsLineItems[1].textContent).toBe('Wing: 55 - 65 (avg 59.9)');
    expect(statsLineItems[2].textContent).toBe('Weight: 11.1 - 15.1 (avg 13.0)');
    expect(statsLineItems[3].textContent).toBe('No notably old birds');
    expect(statsLineItems[4].textContent).toBe('Most caught birds: 3 encounters each BVB4353  ADZ0566  BVB4581  BVB4401 ');
    expect(statsLineItems[5].textContent).toBe('Top sessions: 4  on  3 Dec 2025 3  on  16 Dec 2025 3  on  9 Nov 2025 2  on  20 Dec 2025 2  on  8 Nov 2025 ');
  });
  ...
  it.skip('should allow each individual bird to be expanded', async () => {
    ...
    const expandButton = getByRole(getAllByRole(targetRowEl, 'cell')[0], 'button');
    ...
    const afterRowEls = getAllByRole(topLevelBodyEl, 'row').filter((row) =>
      row.parentElement === topLevelBodyEl);
