...
describe('Postgres RPC integration tests', () => {
	...
	describe('aggregate_stats', () => {
		...
		// first_summer_bird_count / oldies_bird_count. These partition the adult bucket
		...
		describe('age split (new_adult_bird_count / first_summer_bird_count / oldies_bird_count)', () => {
			let deltaId: number;
			let deltaClient: SupabaseClient;
			const locationIds: number[] = [];
			const sessionIds: number[] = [];
			...
			type EncInput = { date: string; age_code: number; is_juv: boolean; record_type: string };

			// Scenario "this period" query dates (each bird gets its own).
			let naDate: string; // new_adult
			let fsDate: string; // first_summer
			let oldDate: string; // oldies (adult returner)
			let tieDate: string; // oldies via a prior-year tie
			let gapDate: string; // oldies via no prior-year encounters
			...
			beforeAll(async () => {
				...
				const { data: deltaLoc, error: dLocErr } = await deltaClient
					.from('Locations')
					.insert({ location_name: `Age Split Delta Loc ${testSuffix}`, ringing_group_id: deltaId })
					.select('id')
					.single();
				...
				const { data: alphaLoc, error: aLocErr } = await alphaClient
					.from('Locations')
					.insert({ location_name: `Age Split Alpha Loc ${testSuffix}`, ringing_group_id: alphaId })
					.select('id')
					.single();
				...
				async function addBird(encounters: EncInput[]): Promise<number> {
					const { data: bird, error } = await deltaClient
						.from('Birds')
						.insert({ ring_no: `SPLIT-${testSuffix}-${ringCounter++}`, species_id: robinId })
						.select('id')
						.single();
					if (error) throw error;
					birdIds.push(bird!.id);
					for (const e of encounters) {
						const sessionId = await getSession(deltaClient, e.date, deltaLocId, sessionIds);
				...
				// oldies: first-ever year earlier; (period_year - 1) majority NOT age_code
				// IN (1,3) (adult retraps); adult this period.
				oldDate = addDays(base, 2);
				const oldPy = yearOf(oldDate) - 1;
				await addBird([
					{ date: `${oldPy}-05-10`, ...ADULT, record_type: 'R' },
					{ date: `${oldPy}-05-11`, ...ADULT, record_type: 'R' },
					{ date: oldDate, ...ADULT, record_type: 'R' },
				]);

				// oldies via a tie: (period_year - 1) exactly one age_code IN (1,3) and one
				// not — a tie falls to oldies, not first_summer.
				...
				// oldies via a gap year: first-ever year is two years back, and there are
				// zero encounters in (period_year - 1) — no prior-year data falls to oldies.
				...
				await insertEncounter(alphaClient, mgBird!.id, alphaSess, { ...ADULT, record_type: 'N' });
				const mgDeltaSess = await getSession(deltaClient, mgDate, deltaLocId, sessionIds);
				await insertEncounter(deltaClient, mgBird!.id, mgDeltaSess, { ...ADULT, record_type: 'N' });
			});
			...
			it('an adult returner whose (period_year - 1) encounters are a majority NOT age_code IN (1,3) counts in oldies_bird_count only', async () => {
				const row = await splitRow(oldDate);
				expect(row).toMatchObject({
					adult_bird_count: 1,
					new_adult_bird_count: 0,
					first_summer_bird_count: 0,
			...
			// Structure
			it('for a cell holding one bird of each adult sub-kind, new_adult + first_summer + oldies = adult_bird_count', async () => {
				const { data, error } = await deltaClient.rpc('aggregate_stats', {
					ringing_group_filter: deltaId,
					from_date: naDate,
					to_date: oldDate,
				...
				expect(row.oldies_bird_count).toBe(1);
				expect(
					row.new_adult_bird_count + row.first_summer_bird_count + row.oldies_bird_count
			...
			// Edge
			it('a (period_year - 1) tie between age_code IN (1,3) and not falls to oldies_bird_count, not first_summer_bird_count', async () => {
			...
			it('a bird with zero encounters in (period_year - 1) (a gap year) falls to oldies_bird_count', async () => {
		...
		describe('young trends (postjuv_juv_enc_count / new_postjuv_juv_enc_count / new_postjuv_enc_count)', () => {
			...
			beforeAll(async () => {
				...
				const { data: location, error: locErr } = await deltaClient
					.from('Locations')
					.insert({ location_name: `Young Trends Loc ${testSuffix}`, ringing_group_id: deltaId })
					.select('id')
					.single();
				...
				async function addBird(
					...
					const { data: bird, error } = await deltaClient
						.from('Birds')
						.insert({ ring_no: `YT-${testSuffix}-${ringCounter++}`, species_id: robinId })
						.select('id')
						.single();
					if (error) throw error;
					birdIds.push(bird!.id);
